...
0. ~~**There is a dependency ceiling, and it is circular.**~~ → **closed between C6 and C7.** The
   `lifecycle-*-compose` artifacts of 2.11 required AGP 9.1 + compileSdk 37, which Jetifier blocked,
   which `swipy` and `pinchtozoom` blocked. C1 and C6 removed both, so AGP 9.3.1 / compileSdk 37
   landed and the `resolutionStrategy.force` came out. Nothing runs a release behind any more.
